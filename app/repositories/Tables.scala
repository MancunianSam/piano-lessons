...
trait Tables {
  ...
  implicit def GetResultNewsRow(implicit e0: GR[java.util.UUID], e1: GR[java.sql.Date], e2: GR[String]): GR[NewsRow] = GR{
    prs => import prs._
  ...
  class News(_tableTag: Tag) extends profile.api.Table[NewsRow](_tableTag, "news") {
    ...
    def ? = ((Rep.Some(id), Rep.Some(date), Rep.Some(title), Rep.Some(body))).shaped.<>({r=>import r._; _1.map(_=> NewsRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
