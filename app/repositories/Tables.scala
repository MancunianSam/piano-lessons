...
trait Tables {
  ...
  case class StudentRow(
      ...
      email: String,
      name: String,
      student: Option[String] = None,
      level: Option[String] = None,
      phone: String,
      notes: Option[String] = None,
      paymentIntentId: Option[String] = None,
      totalCost: Option[scala.math.BigDecimal] = None,
      paymentConfirmed: Option[Boolean] = None
  )

  /** GetResult implicit for fetching StudentRow objects using plain SQL queries */
  implicit def GetResultStudentRow(implicit
      e0: GR[java.util.UUID],
      e1: GR[String],
      e2: GR[Option[String]],
      e3: GR[Option[scala.math.BigDecimal]],
      e4: GR[Option[Boolean]]
  ): GR[StudentRow] = GR { prs =>
    import prs._
    StudentRow.tupled(
      (<<[java.util.UUID], <<[String], <<[String], <<?[String], <<?[String], <<[String], <<?[String], <<?[String], <<?[scala.math.BigDecimal], <<?[Boolean])
    )
  }
  ...
  class Student(_tableTag: Tag) extends profile.api.Table[StudentRow](_tableTag, "student") {
    ...
    def ? = ((Rep.Some(id), Rep.Some(email), Rep.Some(name), student, level, Rep.Some(phone), notes, paymentIntentId, totalCost, paymentConfirmed)).shaped.<>(
      { r => import r._; _1.map(_ => StudentRow.tupled((_1.get, _2.get, _3.get, _4, _5, _6.get, _7, _8, _9, _10))) },
      (_: Any) => throw new Exception("Inserting into ? projection not supported.")
    )
  ...
  case class TimesRow(
      ...
      studentId: Option[java.util.UUID] = None
  )

  /** GetResult implicit for fetching TimesRow objects using plain SQL queries */
  implicit def GetResultTimesRow(implicit e0: GR[java.util.UUID], e1: GR[Int], e2: GR[java.sql.Timestamp], e3: GR[Option[java.util.UUID]]): GR[TimesRow] = GR { prs =>
    import prs._
  ...
  class Times(_tableTag: Tag) extends profile.api.Table[TimesRow](_tableTag, "times") {
    ...
    def ? = ((Rep.Some(id), Rep.Some(numberOfLessons), Rep.Some(lengthOfLessons), Rep.Some(startDate), Rep.Some(endDate), studentId)).shaped.<>(
      { r => import r._; _1.map(_ => TimesRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6))) },
      (_: Any) => throw new Exception("Inserting into ? projection not supported.")
    )
