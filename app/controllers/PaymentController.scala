...
class PaymentController @Inject() (
  ...
  def paymentConfirmation(paymentIntentId: Option[String]): Action[AnyContent] = Action.async { implicit request: Request[Any] =>
    timesRepository
      .getTimes(paymentIntentId.getOrElse(""))
      .map(times => {
        val dates = times
          .map(_._1)
  ...
  def paymentIntent(): Action[JsValue] = Action.async(parse.json) { implicit request: Request[JsValue] =>
    request.body
      .validate[IntentInput]
      .fold(
        errors => Future.successful(BadRequest(Json.obj("message" -> JsError.toJson(errors)))),
        intentInput => {
          studentRepository
            .getStudent(intentInput.studentId)
            .map(studentList => {
              val intentId = for {
              ...
              val paymentIntent: PaymentIntent = intentId match {
                ...
                case None =>
                  stripeConfiguration.paymentIntent(amountService.calculateAmount(intentInput.numberOfLessons, intentInput.lengthOfLesson), intentInput.studentId)
              }
