...
class BookingController @Inject() (
  ...
  def calendar(numberOfLessons: Int, lessonLength: Int, monthOpt: Option[Int], yearOpt: Option[Int]): Action[AnyContent] = Action { implicit request: Request[Any] =>
    ...
    if (month < thisMonth || month == 12) {
      ...
      Redirect(routes.BookingController.calendar(numberOfLessons, lessonLength, Option(thisMonth), Option(thisYear)))
    } else {
  ...
  def saveBookingContactDetails(numOfLessons: Int, lessonLength: Int, date: String, time: String): Action[AnyContent] = Action.async {
    implicit request: MessagesRequest[Any] =>
      contactForm
        .bindFromRequest()
        .fold(
          err => {
          ...
          contact => {
            if (lessonLength > 0) {
              bookingService
                .createPaidBooking(numOfLessons, lessonLength, date, time, contact)
                .map(id => {
                  Redirect(routes.BookingController.bookingSummary(numOfLessons, lessonLength, date, time, id))
                })
            } else {
              bookingService
                .createFreeBooking(1, date, time, contact)
                .map(booking => {
  ...
  def bookingSummary(numOfLessons: Int, lengthOfLesson: Int, date: String, time: String, studentId: UUID): Action[AnyContent] = Action.async {
    implicit request: Request[Any] =>
      ...
      bookingService
        .createBooking(numOfLessons, lengthOfLesson, date, time, studentId, cost)
